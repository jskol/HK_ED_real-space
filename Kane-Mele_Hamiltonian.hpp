...
)
{
    ...
    for(auto spin :spins_set){
        for(int sub=0; sub< params.num_of_sites; sub += num_of_sublattices) {
            H += -sign*2.*params.hopping[1]*sin(params.k)*c_dag(sub,spin)*c(sub,spin);
            H += sign*2.*params.hopping[1]*sin(params.k)*c_dag(sub+1,spin)*c(sub+1,spin);
            H += 2.*params.hopping[0]*cos(0.5*params.k)*c_dag(sub,spin)*c(sub+1,spin);
            H += 2.*params.hopping[0]*cos(0.5*params.k)*c_dag(sub+1,spin)*c(sub,spin);
            if(sub < params.num_of_sites-num_of_sublattices){
                H +=   sign*2.*params.hopping[1]*sin(0.5*params.k)*(c_dag(sub,spin)*c(sub+num_of_sublattices,spin)+c_dag(sub+num_of_sublattices,spin)*c(sub,spin));        
                H +=  -sign*2.*params.hopping[1]*sin(0.5*params.k)*(c_dag(sub+1,spin)*c(sub+1+num_of_sublattices,spin)+c_dag(sub+1+num_of_sublattices,spin)*c(sub+1,spin));          
